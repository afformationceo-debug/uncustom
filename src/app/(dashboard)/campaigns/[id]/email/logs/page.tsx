...
export default function EmailLogsPage() {
  ...
  useRealtime("email_logs", `campaign_id=eq.${campaignId}`, fetchLogsCallback);
  ...
  async function handleResendFailed() {
    ...
    try {
      ...
      const templateGroups = new Map<
        string,
        { templateId: string; influencerIds: string[]; roundNumber: number }
      >();
      for (const log of logsToResend) {
        if (!log.template_id) continue;
        if (!templateGroups.has(log.template_id)) {
          templateGroups.set(log.template_id, {
            templateId: log.template_id,
            influencerIds: [],
            roundNumber: log.round_number,
          });
        }
        templateGroups.get(log.template_id)!.influencerIds.push(log.influencer_id);
      }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          {maxRound > 0 && (
            <Link
              href={`/campaigns/${campaignId}/email/send`}
            >
