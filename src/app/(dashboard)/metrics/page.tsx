...
function MetricsPageContent() {
  ...
  async function refreshMetrics() {
    if (!campaignId) {
      toast.error("메트릭을 새로고침하려면 캠페인을 선택하세요.");
      return;
    }
    setRefreshing(true);
    try {
  ...
  const chartData = useMemo(() => {
    const data = filteredUploads
      ...
      .map((u) => ({
        ...
        campaignName: campaignMap.get(u.campaign_id) ?? "",
      }))
  ...
  }, [filteredUploads, chartMetric, campaignMap]);
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" onClick={refreshMetrics} disabled={refreshing || !campaignId}>
        ...
        <CardContent className="p-0">
          <Table>
            ...
            <TableBody>
              {loading ? (
                <TableRow>
                  <TableCell colSpan={!campaignId ? 8 : 7} className="text-center py-8 text-muted-foreground">
                    로딩 중...
                  </TableCell>
                </TableRow>
                      ...
                      {!campaignId && (
                        <TableCell>
                          <Badge variant="outline" className="text-xs">
                            {campaignMap.get(upload.campaign_id) ?? "알 수 없음"}
                          </Badge>
                        </TableCell>
                      )}
                      <TableCell>
