...
export default function MasterKeywordsPage() {
  ...
  async function fetchKeywords() {
    setLoading(true);
    // Fetch all keywords (master level = campaign_id is null OR all)
  ...
  const grouped = keywords.reduce(
    (acc, kw) => {
      if (!acc[kw.platform]) acc[kw.platform] = [];
      acc[kw.platform].push(kw);
      return acc;
    },
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          인플루언서 추출에 사용할 키워드를 플랫폼별로 등록합니다. 전체 캠페인에서 공유됩니다.
        </p>
      </div>
              ...
              {loading ? (
                ...
                keywords.map((kw) => (
                  <TableRow key={kw.id}>
                    ...
                    <TableCell>
                      <Badge className={platformColors[kw.platform] ?? ""} variant="secondary">
                    ...
                    <TableCell>
                      {!kw.campaign_id && (
                        <Button variant="ghost" size="sm" onClick={() => handleDelete(kw.id)}>
